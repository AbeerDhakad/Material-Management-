$(document).ready(function(){

    $.getJSON("/fetchallstates",{ajax:true},function(data){
     $.each(data,function(index,item){
        $('#state').append($('<option>').text(item[1]).val(item[0]))
     })
 })

 $('#state').change(function () {

 $.getJSON("/fetchallcities",{ajax:true,stateid:$('#state').val()},function(data){
     $('#city').empty()
        $('#city').append($('<option>').text('-City-'))
     $.each(data,function(index,item){
        $('#city').append($('<option>').text(item[2]).val(item[1]))
     })
 })
 })
    $.getJSON("/fetchallgraduation",{ajax:true},function(data){
     $.each(data,function(index,item){
        $('#graduation').append($('<option>').text(item[1]).val(item[0]))
     })
 })
$('#graduation').change(function () {

 $.getJSON("/fetchallcourse",{ajax:true,stateid:$('#graduation').val()},function(data){
     $('#course').empty()
        $('#course').append($('<option>').text('-course-'))
     $.each(data,function(index,item){
        $('#course').append($('<option>').text(item[2]).val(item[1]))
     })
 })
 })
    $('#picture').change(function(){

        var file=picture.files[0]
        pic.src=URL.createObjectURL(file)

    })
})
